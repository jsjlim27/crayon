...
/****************** functions *******************/
static SDL_FRect color_palette_ui_region(SDL_Point win_size)
{
        // want this ui on the left side, centered, 
        // and pretty thin relative to window width.
        float swatch_width = 
                (win_size.x < win_size.y) ? 0.04f * win_size.x : 
                                            0.04f * win_size.y;

        return (SDL_FRect){
                .x = win_size.x - swatch_width,
                .y = 0.5f * win_size.y - 0.5f * swatch_width * COLOR_COUNT,
                .w = swatch_width,
                .h = swatch_width * COLOR_COUNT
        };
}

static UiRegion hit_test(SDL_FPoint p, App *app)
{
        SDL_FRect palette_region = 
                color_palette_ui_region(app->window_size);

        if (SDL_PointInRectFloat(&p, &palette_region)) {
                return REGION_COLOR_PALETTE;
        }
        return REGION_CANVAS;
}

static int palette_swatch_at(SDL_FPoint p, App *app)
{
        SDL_FRect region =
                color_palette_ui_region(app->window_size);

        float y = p.y - region.y;
        float dh = region.h / COLOR_COUNT;
        return (int)(y / dh);
}
 ...
 */
static SDL_FPoint screen_to_world(SDL_FPoint p, SDL_FPoint offset)
{
        return (SDL_FPoint){ p.x - offset.x,
                             p.y - offset.y };
}

static SDL_FPoint world_to_screen(SDL_FPoint p, SDL_FPoint offset)
{
        return (SDL_FPoint){ p.x  + offset.x, 
                             p.y  + offset.y };
}
...
static bool app_init(App *app)
{
        ...
        app->bg_color = PRESET_COLORS[COLOR_BLACK];
        app->brush_color = PRESET_COLORS[COLOR_WHITE];

        app->brush_width = 12.0f;
...
static void app_stroke_begin(App *app, SDL_FPoint p, StrokeInput from)
{
        ...
        point_pool_add(&app->pool, screen_to_world(p, app->position_offset));
...
static void app_stroke_extend(App *app, SDL_FPoint p)
{
        ...
        point_pool_add_segment(
                &app->pool, 
                screen_to_world(app->last_point, app->position_offset),
                screen_to_world(p, app->position_offset),
                2.0f
        );
...
{
        app->brush_width += 2.0f * delta;
        if (app->brush_width < 10.0f) {
...
static void app_panning_extend(App *app, SDL_FPoint rel)
{
        if (!app->panning) { return; }
        app->position_offset.x += rel.x;
        app->position_offset.y += rel.y;
        app->redraw = true;
}
...
static void handle_event(SDL_Event *event, App *app)
{
        ...
        case SDL_EVENT_MOUSE_BUTTON_DOWN:
                ...
                case SDL_BUTTON_LEFT:
                        switch (hit_test(p, app)) {
                        case REGION_COLOR_PALETTE:
                                app->brush_color = 
                                        PRESET_COLORS[palette_swatch_at(p, app)];
                                break;
                        case REGION_CANVAS:
                        ...
                        SDL_FPoint       offset)
{
        ...
        SDL_SetTextureColorMod(tex, stroke->color.r, stroke->color.g, stroke->color.b);

        int end = stroke->start + stroke->length;
        float half = stroke->width / 2;

        for (int i = stroke->start; i < end; i++) {
                // individual stamp's position
                SDL_FPoint p = world_to_screen(pool[i], offset);
                
                // screen area occupied by the stamp
                SDL_FRect area = { .x = p.x - half,
                                   .y = p.y - half,
                                   .w = stroke->width,
                                   .h = stroke->width };

                SDL_RenderTexture(rend, tex, NULL, &area);
        }
}

static void draw_color_palette_ui(SDL_Renderer *rend,
                                  const SDL_Point *win_size)
{
        SDL_FRect region = color_palette_ui_region(*win_size);
        for (int i = 0; i < COLOR_COUNT; i++) {
                SDL_FRect subregion;
                subregion.x = region.x;
                subregion.y = region.y + i * region.h / COLOR_COUNT;
                subregion.w = region.w;
                subregion.h = region.h / COLOR_COUNT;
                SDL_SetRenderDrawColor(rend,
                                       PRESET_COLORS[i].r,
                                       PRESET_COLORS[i].g,
                                       PRESET_COLORS[i].b,
                                       PRESET_COLORS[i].a);
                SDL_RenderFillRect(rend, &subregion);
        }
}
...
static void render(App *app)
{
        ...
        draw_color_palette_ui(app->renderer, &app->window_size);
